...
def api_v1_apps_create(*, session: Session = Depends(get_session), app: AppBase) -> Union[None, App]:
...
def api_v1_apps_update(*, session: Session = Depends(get_session), id: int, app_update: AppBase = ...) -> App:
    return App.update_patch(id, app_update, session)


@router.patch("/api/v1/apps/{id}/", response_model=App)
def api_v1_apps_partial_update(*, session: Session = Depends(get_session), id: int, app_patch: AppPatch = ...) -> App:
